...
int main(int argc, char** argv)
{
    ...
    text_t temperatur_text = text_create("", 30, BLACK);
    text_set_y         (&temperatur_text, GetScreenHeight() * 0.1f);

    text_t spin_text = text_create("", 20, BLACK);
    text_set_y         (&spin_text, GetScreenHeight() * 0.7f);

    text_t binary_text = text_create("1", 40, RED);
    text_set_y         (&binary_text, GetScreenHeight() * 0.75f);

    text_t decimal_text = text_create("", 40, RED);
    text_set_y         (&decimal_text, GetScreenHeight() * 0.9f);

    // ------ magnetic data ------
    magnetic_data_t my_data = magnetic_data_create(N_mag_domains, 42);
